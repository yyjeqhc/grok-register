...
class ProtocolOAuthClient:
    ...
    def load_cookies(self, cookies: Dict[str, str]) -> None:
        ...
        for name, value in cookies.items():
            for dom in ("accounts.x.ai", ".accounts.x.ai", None):
                try:
                    if dom:
                        self._s.cookies.set(name, value, domain=dom)
                    else:
                        self._s.cookies.set(name, value)
                except Exception:
                    pass
        self._log(f"loaded {len(cookies)} cookies into OAuth session (cross-domain optimized)")
    ...
    def _set_sso_cookie(self, jwt_token: str) -> None:
        ...
        for dom in ("accounts.x.ai", ".accounts.x.ai", None):
            try:
                if dom:
                    self._s.cookies.set("sso", jwt_token, domain=dom)
                else:
    ...
    def create_session(self, email: str, password: str, *, referer: str) -> Dict[str, Any]:
        ...
        website_url = "https://accounts.x.ai/"
    ...
    def login(
        ...
        *,
        ...
        # Match official Grok Build CLI authorize params so the access_token may
        # carry JWT claim referrer=grok-build (observed on free-path captures).

#!/usr/bin/env python3
"""Batch mint CPA xai-*.json from register account output files.
...
from cpa_xai import existing_cpa_emails, mint_and_export, parse_account_files  # noqa: E402


def discover_account_files(root: Path) -> list[Path]:
    files = [*root.glob("accounts_*.txt"), *root.glob("accounts_*.jsonl")]
    return sorted(files, key=lambda path: (path.stem, path.suffix, path.name))
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        nargs="+",
        default=None,
        help="Account TXT/JSONL files; default discovers accounts_* under the project root",
    )
    ap.add_argument(
        "--out-dir",
        ...
        try:
            ...
            if cfg_path.is_file():
                ...
                if isinstance(cfg, dict):
                    ...
                    if not args.cpa_dir and cfg.get("cpa_copy_to_hotload", False):
    ...
    account_paths = [Path(path) for path in args.accounts] if args.accounts else discover_account_files(_ROOT)
    accounts = parse_account_files(account_paths)
    if args.email:
    ...
    print(f"files={len(account_paths)} accounts total={len(accounts)} todo={len(todo)} out={args.out_dir}")
